...
/*   Updated: 2020/12/12 12:54:20 by scolen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf.h"

static void	hex_continue_h(char *ptr, int count_devision,
	long number, t_varible *g_varible)
{
	int start;
	int new_len_nbr;
	int new_width;
	...
	{
		ptr[start++] = ft_toupper(value_hex(number % 16));
		number = number / 16;
	}
...
void		threatment_hex_h(const char *s, va_list *va_args,
	t_varible *g_varible)
{
	long	number;
	char	*ptr;
